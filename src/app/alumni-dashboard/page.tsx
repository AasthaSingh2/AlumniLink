  ...
  const quickActions = [
    {
      ...
      description: 'Quickly import your professional details',
      icon: Linkedin,
      href: '/alumni-profile?import=linkedin',
      color: 'bg-blue-600',
      isHighlighted: true
    },
    {
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {quickActions.map((action, index) => (
                    <Card
                      key={index}
                      className={`hover:shadow-md transition-shadow ${action.isHighlighted ? 'border-2 border-blue-400' : ''}`}
                    >
                      <a href={action.href}>
                        <CardContent className="p-6">
                          <div className="flex items-center space-x-4">
                            <div className={`p-3 rounded-lg ${action.color} text-white ${action.isHighlighted ? 'animate-pulse' : ''}`}>
            ...
            <Card className="bg-background-light border-border">
              ...
              <CardContent className="space-y-4">
                {notifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`p-4 rounded-lg border ${notification.unread ? 'bg-accent/10 border-accent/20' : 'bg-background-lighter border-border'
                      }`}
                  >
